...
import com.lzj.facetoface.common.DeleteRequest;
...
import com.lzj.facetoface.model.request.TeamAddRequest;
...
public class TeamController {
    ...
    public BaseResponse<List<TeamUserVO>> listTeams(TeamQuery teamQuery, HttpServletRequest request){
        ...
        /*for (UserTeam user: userTeamList) {
            System.out.println(user);
        }*/
        // 队伍id => 加入这个队伍的用户列表
        Map<Long, List<UserTeam>> teamIdUserTeamList = userTeamList.stream().collect(Collectors.groupingBy(UserTeam::getTeamId));
        System.out.println("队伍id => 加入这个队伍的用户列表：" + teamIdUserTeamList);
        teamList.forEach(team -> {
    ...
    public BaseResponse<Boolean> joinTeam(@RequestBody TeamJoinRequest teamJoinRequest, HttpServletRequest request) {
        ...
        User loginUser = userService.getLoginUser(request);
        boolean joinResult = teamService.joinTeam(teamJoinRequest, loginUser);
        return ResultUtils.success(joinResult);
    }
    ...
    public BaseResponse<Boolean> quitTeam(@RequestBody TeamQuitRequest teamQuitRequest, HttpServletRequest request) {
        ...
        User loginUser = userService.getLoginUser(request);
        boolean quitResult = teamService.quitTeam(teamQuitRequest, loginUser);
        return ResultUtils.success(quitResult);
    }
    ...
    public BaseResponse<Boolean> deleteTeam(@RequestBody DeleteRequest deleteRequest, HttpServletRequest request){
        ...
        User loginUser = userService.getLoginUser(request);
        boolean deleteResult = teamService.deleteTeam(id, loginUser);
        if (!deleteResult) {
    ...
    public BaseResponse<List<Team>> myCreateTeams(TeamQuery teamQuery, HttpServletRequest request) {
        if (teamQuery == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        User loginUser = userService.getLoginUser(request);
        ...
        List<Team> teamList = teamService.list(queryWrapper);
        if (teamList.size() <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "该用户没有创建的队伍");
        }
        return ResultUtils.success(teamList);
    }*/
